...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Baseline B: queue-imbalance-only logistic regression."
    )
    ...
    parser.add_argument(
        "--save-predictions",
        action="store_true",
        help="Save row-level train/validation predictions. Can be large.",
    )
...
def run_queue_imbalance_logistic(
        data: pd.DataFrame,
    ...
    """
    Runs queue-imbalance-only logistic regression for each horizon.
...
def main() -> None:
    ...
    print("=" * 80)
    print("STEP 11: BASELINE B — QUEUE-IMBALANCE-ONLY LOGISTIC REGRESSION")
    print("=" * 80)
    ...
    print()
    print("Running queue-imbalance-only logistic regression on train and validation...")

    tables, nonzero_table, coefficients, predictions = run_queue_imbalance_logistic(
        data=data,
        config=config,
    ...
    print()
    print("Done. Queue-imbalance logistic baseline completed.")
