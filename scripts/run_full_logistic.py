...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fit full-feature logistic models with bounded-memory validation "
            "and transactional per-horizon checkpoints."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Protocol horizons to fit. Default: 10 20 50.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=1_000_000,
        help="Rows per validation and metric-accumulation batch.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Reuse only complete, strictly compatible horizon checkpoints.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Deliberately replace existing model artifacts and checkpoints.",
    )
    parser.add_argument(
        "--require-clean-git",
        action="store_true",
        help="Fail unless the source worktree is clean.",
    )
...
def main() -> None:
    ...
    with RunRecorder(
        ...
        print("=" * 80)
        print("PRIMARY MODEL: FULL-FEATURE LOGISTIC REGRESSION")
        print("=" * 80)
        ...
        print("Done. Full-feature logistic models completed.")
